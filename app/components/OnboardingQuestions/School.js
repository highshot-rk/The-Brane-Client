...
export default class School extends Component {
  ...
  onClick = () => {
    if (this.props.institutions.length === 0) {
      this.setState({
        error: 'Please enter schools, universities, colleges, etc ',
      })
    } else {
  ...
  render () {
    const { onContinue, onPrevious, institutions, onChange, addTag, later } = this.props
    return (
      <QuestionContainer>
        <QuestionForm>
          <div className='q-width'>
            <a onClick={onPrevious} className='navigation'>Previous</a>
            <h5>Question 2 of 8</h5>
            <p className='title'>Where did you study?<br />Select all relevant institutions.</p>
          </div>
          <div className='q-width'>
            <InputRow className='input-style' >
              <p className='input-label'>Type the name of your schools, universities, colleges, etc</p>
              <YagoInputTag question={'education'} placeholder={'Education'} query={institutions} tags={institutions} removeTag={onChange} addTag={addTag} />
